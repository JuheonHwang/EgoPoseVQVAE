...
from model.self_pose4 import SelfPose
...
from dataset import Mocap4 as Mocap
from utils import config, ConsoleLogger
from utils import evaluate, io


def train_model(model, dataloaders, device, logger,
                pose_loss, rot_loss, heatmap_loss, heatmap_loss2,l1_loss,
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            sizes = {"train":0, "val":0}
            i = 0
            for image, p2d, gt_pose, action, gt_hm, gt_rot in dataloaders[phase]:
...
#                     _, preds = torch.max(outputs, 1)
                    ...
                    pred_rot = pred_rot.view(-1, 16, 3)
                    gt_rot = gt_rot.view(-1, 16, 3)
                    r_loss = rot_loss(pred_pose, gt_pose)
...
#                     l1_loss_pose = l1_loss(pred_pose, gt_pose)
                    ...
                    loss = p_loss + (r_loss/10) + (hmp_loss/50) + (hmp_emb_loss/50)
                ...
                if i % 10 == 0:
                    ...
                    logger.info(f"l2 pose {p_loss.item()} rot {r_loss.item()} hmp loss {hmp_loss.item()} hmp2 {hmp_emb_loss.item()} LR: {scheduler.get_last_lr()}")# "l1 loss",l1_loss_pose.item(), "cos loss",cosine_loss.item())
                i += 1
                if i % 5000 == 0:
                    torch.save(model.state_dict(), f"./ckpts/model4/epoch{epoch}_iter{i}.pth")
...
#             epoch_acc = running_corrects.double() / dataset_sizes[phase]
        ...
        print()
        torch.save(model.state_dict(), f"./ckpts/model4/{epoch}_epoch_{epoch_loss}.pth")
...
def main():
    ...
    model.to(device)
    logging.basicConfig(filename="./train_logs/model4.log", format='%(message)s',
                        level=logging.INFO, filemode='w')
    logger = logging.getLogger('train_logger')
    print("loaded", device)

    best = train_model(model, dataloaders, device, logger,
                       pose_criterion, rot_loss, hmp_loss, hmp_loss2, l1_loss,
