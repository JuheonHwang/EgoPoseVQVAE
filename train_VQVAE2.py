    ...
    for epoch in range(num_epochs):
        ...
        for gt_pose in dataloader:
            ...
            with torch.set_grad_enabled(True):
                ...
                i += 1
                if i % 5000 == 0:
                    torch.save(model.state_dict(), f"./ckpts/model_vqvae2/epoch{epoch}_iter{i}.pth")
            
        scheduler.step()
        epoch_loss = running_loss / size
        print('Loss: {:.4f}'.format(epoch_loss))           
 
        torch.save(model.state_dict(), f"./ckpts/model_vqvae2/{epoch}_epoch_{epoch_loss}.pth")
...
def main():
    ...
    logging.basicConfig(filename="./train_logs/vqvae2.log", format='%(message)s',
