...
#                     l1_loss_pose = l1_loss(pred_pose, gt_pose)
                ...
                i += 1
                if i % 5000 == 0:
                    torch.save(model.state_dict(), f"./ckpts/model6/epoch{epoch}_iter{i}.pth")
...
#             epoch_acc = running_corrects.double() / dataset_sizes[phase]
        ...
        print()
        torch.save(model.state_dict(), f"./ckpts/model6/{epoch}_epoch_{epoch_loss}.pth")
...
def main():
    ...
    model.to(device)
    logging.basicConfig(filename="./train_logs/model6.log", format='%(message)s',
