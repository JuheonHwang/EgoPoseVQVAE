...
from model.self_pose5 import SelfPose
...
from dataset import Mocap5 as Mocap
from utils import config4 as config
...
#                     _, preds = torch.max(outputs, 1)
                    ...
                    pred_rot = pred_rot.view(-1, 14, 3)
                    gt_rot = gt_rot.view(-1, 14, 3)
...
#                     l1_loss_pose = l1_loss(pred_pose, gt_pose)
                ...
                i += 1
                if i % 5000 == 0:
                    torch.save(model.state_dict(), f"./ckpts/model5/epoch{epoch}_iter{i}.pth")
...
def main():
    ...
    model.to(device)
    logging.basicConfig(filename="./train_logs/model5.log", format='%(message)s',
