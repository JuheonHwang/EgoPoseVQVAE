...
def visualization(pred_pose, gt_pose, p_loss, inference_time):
    ...
    linewidth = 4
    kinetic_chain = [[1, 2, 3, 4, 5, 0], [5, 6, 7, 8, 9], [5, 10, 11, 12, 13], [1, 14, 15, 16, 17], [1, 18, 19, 20, 21]]
...
def main():
    ...
    model.load_state_dict(torch.load("./ckpts/model3/epoch6_iter15000.pth"))
    model.to(device)
    model.eval()
    ...
    for image, p2d, gt_pose, action, gt_hm in test_loader:
        ...
        pred_pose = pred_pose.detach().cpu().view(-1, 22, 3)
        gt_pose = gt_pose.detach().cpu().view(-1, 22, 3)
