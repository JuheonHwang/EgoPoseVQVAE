...
class Mocap5(BaseDataset):
    ...
    def _get_heatmap(self, p2d, shape, new_shape=(51,51), joints=15): # real new_shape should be (47,47)
        width, height = shape
        new_length = width * (width <= height) + height * (height <= width)
        half_reduced_width = (width - new_length) / 2
        half_reduced_height = (height - new_length) / 2
                
        p2d[:, 0] = ((p2d[:, 0] - half_reduced_width + 1) / new_length) * new_shape[0]
        p2d[:, 1] = ((p2d[:, 1] - half_reduced_height + 1) / new_length) * new_shape[1]
    ...
    def __getitem__(self, index):
        ...
        p2d, p3d, pRot = self._process_points(data)
        
        # get heatmap
        heatmap = self._get_heatmap(p2d[1:, ...], img.size, joints=len(p2d)-1)
        
        # for heatmap visualize
        # h = np.transpose(np.sum(heatmap, axis=0))
        # h = Image.fromarray(h*255)
        # h.show()
        ...
        if self.transform:
            ...
            pRot = self.transform({'jointsRot': pRot})['jointsRot']

        return img, p2d, p3d, action, heatmap, pRot
